...
/**
 * Static index data for all indices (simplified - for production use BACEN API)
 */
const INDICES_ESTATICOS: Record<IndexType, Record<string, number>> = {
  inpc: {
    '2024-01': 0.57, '2024-02': 0.81, '2024-03': 0.19, '2024-04': 0.37,
    '2024-05': 0.46, '2024-06': 0.25, '2024-07': 0.01, '2024-08': -0.14,
  ...
  igpm: {
    '2024-01': 0.07, '2024-02': -0.52, '2024-03': -0.47, '2024-04': 0.31,
    '2024-05': 0.89, '2024-06': 0.81, '2024-07': 0.61, '2024-08': 0.29,
    '2024-09': 0.62, '2024-10': 1.52, '2024-11': 1.30, '2024-12': 0.94,
  },
  incc: {
    '2024-01': 0.27, '2024-02': 0.20, '2024-03': 0.21, '2024-04': 0.52,
    '2024-05': 0.59, '2024-06': 0.71, '2024-07': 0.69, '2024-08': 0.64,
    '2024-09': 0.61, '2024-10': 0.67, '2024-11': 0.44, '2024-12': 0.50,
  },
  ufir: {}, // Extinta em 2000
};

/**
 * Get index from static data (fallback when API is unavailable)
 */
export function getIndexFromStatic(type: IndexType, date: Date): number {
  const key = `${date.getFullYear()}-${(date.getMonth() + 1).toString().padStart(2, '0')}`;
  const indiceData = INDICES_ESTATICOS[type];

  if (!indiceData) {
    return 0;
  }

  // Return as decimal (e.g., 0.57% = 0.0057)
  return (indiceData[key] ?? 0) / 100;
}
